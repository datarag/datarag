...
describe('Retrieve Questions API', () => {
  let factory;
  ...
  describe('POST /retrieve/questions', () => {
    it('should retrieve chunks based on a query', async () => {
      const agent = supertest.agent(app);
      const res = await agent
        .post('/v1/retrieve/questions')
    ...
    it('should handle empty results if no relevant questions found', async () => {
      const agent = supertest.agent(app);
      const res = await agent
        .post('/v1/retrieve/questions')
        .set('Authorization', `Bearer ${TOKEN}`)
    ...
    it('should limit the number of retrieved questions based on max_questions', async () => {
      const agent = supertest.agent(app);
      const res = await agent
        .post('/v1/retrieve/questions')
        .set('Authorization', `Bearer ${TOKEN}`)
