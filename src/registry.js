...
/**
 * Delete a key = require(registr)y
 *
 * @param {String} key
 * @returns {Promise}
 */
 ...
 * @param {String} key
 * @param {*} value
 * @returns {Promise<Boolean>} - whether value was removed
 */
...
async function getLock(key) {
  const lock = await redlock.acquire([keyToRedis(`lock:${key}`)], 5000);
  return lock;
}
